...
export default function ProductVariantCondition() {
    const auth = useAuth();
    ...
    const handlePublishedChange = (index) => {
        const updatedtags = [...conditions];
        updatedtags[index].isPublish = !updatedtags[index].isPublish;
        setConditions(updatedtags);
        console.log(updatedtags[index].isPublish);

        setProgress(50);
        axios
            .patch(
                // ` localhost:4000/publish/product/variantcondition/${conditions[index]._id}`,
                `${auth.ip}publish/product/variantcondition/${conditions[index]._id}`,
                {
    ...
    const handleDelete = (id) => {
        setProgress(50);
        axios
            // .delete(` localhost:4000/delete/product/variantcondition/${id}`)
            .delete(`${auth.ip}delete/product/variantcondition/${id}`)
            .then((response) => {
                if (response.data.status === 201) {
                    toast.success(response.data.message);
                    // setErrState(false);
    ...
    const handleSubmit = (event) => {
        ...
        if (tagName === "") {
            ...
            isEdit
                ? axios
                    // .patch(` localhost:4000/update/product/variantcondition/${tagID}`, {
                    .patch(`${auth.ip}update/product/variantcondition/${tagID}`, {
                        name: tagName,
                    })
                    .then((response) => {
                        if (response.data.status === 201) {
                ...
                : axios
                    .post(`${auth.ip}create/product/variantcondition`, {
                        name: tagName,
                        isPublish: true,
                    })
                    .then((response) => {
    ...
    function getCondition() {
        setProgress(50);
        axios
            .get(`${auth.ip}get/product/variantcondition`)
            .then((response) => {
                if (response.data.status === 201) {
                    ...
                    setErrState(false);
                    console.log(response.data.productConditions);
                    setConditions(response.data.productConditions);
        ...
        <>
            ...
            <div className="sm:flex sm:items-center">
                <div className="sm:flex-auto">
                    {/* <h2 className="text-lg font-bold  text-gray-900">ProductCondition</h2> */}
                </div>
            </div>
                        ...
                        <div className="relative bg-white rounded-lg shadow dark:bg-gray-200">
                            <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t border-purple-200">
                                <h3 className="text-lg font-semibold text-purple-700">
                                    {isEdit ? "Edit Variant Condition" : "Add new Variant Condition"}
                                </h3>
                                <button
