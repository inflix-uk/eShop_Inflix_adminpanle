...
export default function ProductCondition() {
  ...
  const handlePublishedChange = (index) => {
    ...
    axios
      .patch(
        // ` localhost:4000/publish/product/condition/${conditions[index]._id}`,
        `${auth.ip}publish/product/condition/${conditions[index]._id}`,
        {
  ...
  const handleDelete = (id) => {
    ...
    axios
      ...
      .delete(`${auth.ip}delete/product/condition/${id}`)
      .then((response) => {
        if (response.data.status === 201) {
          toast.success(response.data.message);
          // setErrState(false);
    ...
    if (tagName === "") {
      ...
      isEdit
        ? axios
          ...
          .patch(`${auth.ip}update/product/condition/${tagID}`, {
            name: tagName,
          })
          .then((response) => {
            if (response.data.status === 201) {
        ...
        : axios
          .post(`${auth.ip}create/product/condition`, {
            name: tagName,
            isPublish: true,
          })
          .then((response) => {
  ...
  function getCondition() {
    setProgress(50);
    axios
      .get(`${auth.ip}get/product/condition`)
      .then((response) => {
        if (response.data.status === 201) {
                  ...
                  <tr>
                    ...
                    >
                      <button
                        ...
                        type="button"
                        className="inline-flex items-center gap-1.5 rounded-lg bg-rose-100 px-4 py-2.5 text-sm font-medium text-rose-700 hover:bg-rose-200 transition-colors duration-150 shadow-sm"
                      >
