...
export default function ProductColors() {
  ...
  const [tagColorCode, setTagColorCode] = useState("#000000")
  ...
  // Helper function to convert color name to URL-friendly format
  const formatColorForStorage = (colorName) => {
    return colorName.trim().replace(/\s+/g, '-'); // Replace spaces with hyphens
  };

  // Helper function to display color name with spaces on the UI
  const formatColorForDisplay = (colorName) => {
    return colorName.replace(/-/g, ' '); // Replace hyphens with spaces
  };
  const handlePublishedChange = (index) => {
    const updatedtags = [...colors];
    ...
    axios
      .patch(
        `${auth.ip}publish/product/color/${colors[index]._id}`,
        {
  ...
  const handleDelete = (id) => {
    setProgress(50);
    axios.delete(`${auth.ip}delete/product/color/${id}`)
      .then((response) => {
        if (response.data.status === 201) {
          // toast.success(response.data.message);
          // setErrState(false);
  ...
  const handleSubmit = (event) => {
    ...
    if (tagName === "") {
      ...
      const formattedTagName = formatColorForStorage(tagName);
      isEdit
        ? axios
          .patch(`${auth.ip}update/product/color/${tagID}`, {
            name: formattedTagName, // Use formatted tag name
            colorcode: tagColorCode, // send the color code to the API
          })
          .then((response) => {
            if (response.data.status === 201) {
              toast.success(response.data.message);
              setErrState(false);
              set_tagName("");
              setTagColorCode("#000000"); // reset the color code
              set_tagDesc("");
              set_tagMetaTitle("");
              set_tagMetaImage("");
              set_tagMetaDesc("");
        ...
        : axios
          .post(`${auth.ip}create/product/color`, {
              ...
              <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                <thead className="text-xs text-black uppercase border-b border-gray-200">
                  <tr>
                    ...
                    <th
                      scope="col"
                      className="px-3 py-3.5 text-right text-sm font-semibold text-gray-900 flex items-center justify-end"
                    >
                      <button
                        onClick={() => {
                ...
                <tbody className="divide-y divide-gray-200 bg-white">
                  {colors.length > 0 && colors.map((tag, index) => (
                    <tr key={index}>
                      <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">
                        {/* Display formatted name */}
                        {formatColorForDisplay(tag.name)}
                      </td>
                      <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">
                        {tag.colorcode}
                      </td>
        ...
        <div className="fixed inset-0 z-50 flex justify-center items-center bg-black bg-opacity-50 overflow-y-auto">
          ...
          >
            <div className="relative bg-white rounded-lg shadow dark:bg-gray-200">
              <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t border-purple-200">
                <h3 className="text-lg font-semibold text-purple-700">
                  {isEdit ? "Edit Variant Color" : "Add New Variant Color"}
                </h3>
                <button
                  onClick={() => setnewTagsModal(false)}
                  type="button"
                  className="text-gray-400 bg-transparent hover:bg-purple-100 hover:text-purple-700 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center"
                ...
                <div className="grid gap-2 mb-4 grid-cols-2">
                  <div className="col-span-2">
                    ...
                    <input
                      ...
                      name="name"
                      id="name"
                      className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-2.5 dark:border-gray-500 dark:placeholder-gray-400"
                  ...
                  <div className="col-span-2">
                    <label
                      ...
                      className="block mb-2 text-sm font-medium text-gray-900 "
                    >
                      Variant Color <span className="text-red-600">*</span>
                    </label>
                    <input
                      type="color"
                      value={tagColorCode}
                      onChange={(e) => setTagColorCode(e.target.value)}
                    />
                  </div>
                </div>
                <button
                  type="submit"
                  className="text-white inline-flex items-center bg-purple-600 hover:bg-purple-700 focus:ring-4 focus:outline-none focus:ring-purple-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center"
                >
                  <svg
