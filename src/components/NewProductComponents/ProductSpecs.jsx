      ...
      <div className="px-0 shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl">
        <div className="py-4 px-4 space-y-3">
          <div className="flex flex-row items-center justify-between">
            ...
            <button
              className="text-blue-600 bg-gray-50 group flex rounded-lg p-2 gap-2 text-xs leading-6 font-semibold border-gray-300 border"
              onClick={addNewSpec}
            >
              Add New
              <svg
          ...
          {specs.map((spec) => (
            ...
            >
              ...
              <input
                type="text"
                name={`specValue${spec.id}`}
                id={`specValue${spec.id}`}
                className="block w-full rounded-md border-0 py-1.5  text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm sm:leading-6"
                placeholder="50mp"
              />
              <button
                className="text-blue-600 bg-gray-50 group flex gap-x-3 rounded-lg  text-sm leading-6 font-semibold items-center"
