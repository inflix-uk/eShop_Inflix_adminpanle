import { formatMessageTime } from "../utils/helpers";
import { FiUser, FiFile, FiDownload } from "react-icons/fi";

const BACKEND_URL = "http://localhost:4000";

export default function MessageBubble({ message, visitorName }) {
  const isAdmin = message.sender === "admin";
  const isBot = message.sender === "bot";
  const isAdminOrBot = isAdmin || isBot; // Both admin and bot messages on right side
  const isUser = !isAdmin && !isBot;
  const attachments = message.attachments || [];

  // Get initials from visitor name
  const getInitials = (name) => {
    if (!name) return "V";
    return name
      .split(" ")
      .map((n) => n[0])
      .join("")
      .toUpperCase()
      .slice(0, 2);
  };

  // Check if file is an image
  const isImage = (mimetype) => mimetype?.startsWith("image/");

  // Get full URL for attachment
  const getAttachmentUrl = (path) => `${BACKEND_URL}${path}`;

  return (
    <div className={`flex flex-col ${isAdminOrBot ? "items-end" : "items-start"}`}>
      <div className={`flex items-end gap-2 ${isAdminOrBot ? "flex-row-reverse" : ""}`}>
        {/* Admin/Bot Avatar - shown on right side */}
        {isAdminOrBot && (
          <div className="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center flex-shrink-0">
            <svg className="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 24 24">
              <path d="M12 2a2 2 0 012 2c0 .74-.4 1.39-1 1.73V7h1a7 7 0 017 7h1a1 1 0 011 1v3a1 1 0 01-1 1h-1v1a2 2 0 01-2 2H5a2 2 0 01-2-2v-1H2a1 1 0 01-1-1v-3a1 1 0 011-1h1a7 7 0 017-7h1V5.73c-.6-.34-1-.99-1-1.73a2 2 0 012-2M7.5 13A1.5 1.5 0 006 14.5a1.5 1.5 0 001.5 1.5A1.5 1.5 0 009 14.5 1.5 1.5 0 007.5 13m9 0a1.5 1.5 0 00-1.5 1.5 1.5 1.5 0 001.5 1.5 1.5 1.5 0 001.5-1.5 1.5 1.5 0 00-1.5-1.5M12 17a1 1 0 00-1 1 1 1 0 001 1 1 1 0 001-1 1 1 0 00-1-1z"/>
            </svg>
          </div>
        )}

        {/* User Avatar - only for visitor messages */}
        {isUser && (
          <div className="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center text-white text-xs font-semibold flex-shrink-0">
            {visitorName ? getInitials(visitorName) : <FiUser className="w-4 h-4" />}
          </div>
        )}

        {/* Message Bubble */}
        <div
          className={`max-w-[75%] px-4 py-2.5 ${
            isAdminOrBot
              ? "bg-blue-600 text-white rounded-2xl rounded-br-sm"
              : "bg-white text-slate-800 shadow-sm border border-slate-200 rounded-2xl rounded-bl-sm"
          }`}
        >
          {/* Text content */}
          {(message.text || message.message) && (
            <p className="text-sm leading-relaxed">{message.text || message.message}</p>
          )}

          {/* Attachments */}
          {attachments.length > 0 && (
            <div className={`${message.text ? "mt-2" : ""} space-y-2`}>
              {attachments.map((attachment, index) => (
                <div key={index}>
                  {isImage(attachment.mimetype) ? (
                    <a
                      href={getAttachmentUrl(attachment.path)}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="block"
                    >
                      <img
                        src={getAttachmentUrl(attachment.path)}
                        alt={attachment.originalName}
                        className="max-w-[200px] rounded-lg hover:opacity-90 transition-opacity"
                      />
                    </a>
                  ) : (
                    <a
                      href={getAttachmentUrl(attachment.path)}
                      target="_blank"
                      rel="noopener noreferrer"
                      className={`flex items-center gap-2 p-2 rounded-lg ${
                        isAdminOrBot
                          ? "bg-white/20 hover:bg-white/30"
                          : "bg-slate-100 hover:bg-slate-200"
                      } transition-colors`}
                    >
                      <FiFile className="w-4 h-4 flex-shrink-0" />
                      <span className="text-xs truncate max-w-[150px]">
                        {attachment.originalName}
                      </span>
                      <FiDownload className="w-3 h-3 flex-shrink-0 ml-auto" />
                    </a>
                  )}
                </div>
              ))}
            </div>
          )}
        </div>
      </div>

      {/* Time displayed below bubble */}
      <p className={`text-[10px] mt-1 text-slate-400 ${isAdminOrBot ? "mr-10" : "ml-10"}`}>
        {formatMessageTime(message.createdAt)}
      </p>
    </div>
  );
}
